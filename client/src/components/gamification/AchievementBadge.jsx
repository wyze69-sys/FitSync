...
export default function AchievementBadge({ level = 1, title = "Starter", size = "md", badge }) {
  const displayLevel = Number(level || badge?.level || 1);
  const displayTitle = title || badge?.name || "Starter";
    ...
    <article className="animate-badge-pop rounded-3xl border border-border bg-surface p-5 text-center shadow-md" aria-label={`${displayTitle}, level ${displayLevel}`}>
      <div className={`rank-medal rank-medal-${tier} ${sizeClass} mx-auto`}>
        <svg viewBox="0 0 100 118" className="h-full w-full" aria-hidden="true" role="img">
          ...
          <circle className="rank-medal-ring" cx="50" cy="48" r="42" fill={tier === "bronze" ? `url(#${gradientId}-bronze)` : tier === "gold" ? `url(#${gradientId}-gold)` : undefined} />
